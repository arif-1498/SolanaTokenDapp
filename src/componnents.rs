...
use anchor_spl::{
    ...
    token_interface::{mint_to, Mint as MintInterface, MintTo, Token2022, TokenAccount as TokenInterfaceAccount, TokenInterface, Transfer},
   
};
use spl_token_2022::extension::{transfer_fee::{instruction::{initialize_transfer_fee_config, set_transfer_fee, transfer_checked_with_fee, withdraw_withheld_tokens_from_accounts,},
TransferFeeConfig, TransferFee, TransferFeeAmount}
};
...
mod fungible_token {
    use super::*;
    pub fn initialize_token(_ctx: Context<InitializeToken>, _metadata: TokenParams, _fee_bp:u16) -> Result<()> {
    ...
    pub fn mint_token(_ctx: Context<MintTokens>, _amounts: u64) -> Result<()> {
        ...
        let cpi_context =
            CpiContext::new(_ctx.accounts.token_program.to_account_info(), cpi_accounts);
    ...
    pub fn transfer_token(_ctx: Context<TransferToken>, amount: u64) -> Result<()> {
        ...
        transfer_checked_with_fee(token_program_id, source, mint, destination, authority, signers, amount, decimals, fee);
        Ok(())
    }
